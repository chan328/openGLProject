...
glm::mat4 buildViewMatirx(CameraData cam)
{
	using namespace glm;
...
void ofApp::setup(){
	...
	buildMesh(charMesh, 0.125, 0.25, glm::vec3(0.0, -0.2, 0.0));
	buildMesh(backMesh, 1.0, 1.0, glm::vec3(0.0, 0.0, 0.5));
	...
	spritesheetShader.load("sprite.vert", "alphaTest.frag");
	charShader.load("draw.vert", "alphaTest.frag");
	cloudShader.load("draw.vert", "cloud.frag");
}
...
void ofApp::draw(){
	...
	charShader.begin();
	charShader.setUniformTexture("greenMan", backgroundImg,  0);
	charShader.setUniformMatrix4f("transform", translate(vec3(0.0, 0.0, 0.5)));
	backMesh.draw();
	charShader.end();
	...
	cloudShader.begin();
	cloudShader.setUniformTexture("cloud", cloudImg, 0);
